...
### 3. Gate のコストはキャッシュの奪い合いだった

Tool 経路だけのカテゴリ (45 件) で比較すると:
...
+1570ms のうち、推論 1 回分より**キャッシュヒット率の低下 (97%→85%) の寄与が大きい**。
llama-server が `-np 1` (単一スロット) なので、判定用と Loop 用の 2 つの system prompt が
交互に KV キャッシュを追い出し合っている。

→ **サーバを 2 スロット (`--parallel 2`) にすれば大きく縮む見込み。未検証。**
現状は正確さを優先して Gate を既定で有効にしている。
